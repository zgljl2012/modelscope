...
from ..util import (count_params, default, exists, instantiate_from_config,
...
class DDPM(pl.LightningModule):
    ...
    def register_schedule(self,
        ...
        beta_2 = (1. - alphas_cumprod)
        self.register_buffer('posterior_mean_coef1', to_torch(beta_1 / beta_2))
        alpha_1 = (1. - alphas_cumprod_prev) * np.sqrt(alphas)
        alpha_2 = (1. - alphas_cumprod)
        self.register_buffer('posterior_mean_coef2',
                             to_torch(alpha_1 / alpha_2))
    ...
    def q_posterior(self, x_start, x_t, t):
        p_1 = extract_into_tensor(self.posterior_mean_coef1, t,
                                  x_t.shape) * x_start
        p_2 = extract_into_tensor(self.posterior_mean_coef2, t,
                                  x_t.shape) * x_t
...
class LatentDiffusion(DDPM):
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if cond_1 and cond_2 and not self.restarted_from_ckpt:
            assert self.scale_factor == 1., 'error'
    ...
    def get_input(self,
        ...
        with torch.enable_grad():
            ...
            cond['c_crossattn'] = [
                self.cc_projection(
                    torch.cat([mask_used, T[:, None, :]], dim=-1))
            ]
        cond['c_concat'] = [
            input_mask * self.encode_first_stage(
                (xc.to(self.device))).mode().detach()
        ]
        out = [z, cond]
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, 'split_input_params'):
            ...
            if self.cond_stage_key in [
                ...
                res_2 = rescale_latent * stride[1] * (
                    patch_nr // n_patches_per_row) / full_img_h
                tl_patch_coordinates = [(res_1, res_2)
    ...
    def _predict_eps_from_xstart(self, x_t, t, pred_xstart):
        ex_1 = (
            extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape)
            * x_t - pred_xstart)
        ex_2 = extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t,
                                   x_t.shape)
        return ex_1 / ex_2
    ...
    def get_unconditional_conditioning(self,
        ...
        cond['c_crossattn'] = [c]
        cond['c_concat'] = [
            torch.zeros([batch_size, 4, image_size // 8,
                         image_size // 8]).to(self.device)
        ]
        return cond
    ...
    def configure_optimizers(self):
        ...
        if self.unet_trainable == 'conv_in':
            ...
            params = list(
                self.model.diffusion_model.input_blocks[0][0].parameters())
        ...
        param_2 = {'params': self.cc_projection.parameters(), 'lr': 10. * lr}
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            for k in c:
                if k == 'c_crossattn':
                    ...
                    uc[k] = torch.ones_like(
                        c[k]) * self.low_scale_model.max_noise_level
...
class LatentInpaintDiffusion(LatentDiffusion):
            ...
            if exists(self.finetune_keys) and k in self.finetune_keys:
                ...
                for name, param in self.named_parameters():
                    if name in self.finetune_keys:
                        print(
                            f"modifying key '{name}' and keeping its original {self.keep_dims} dimensions only"
                        )
    ...
    def log_images(self,
        ...
        log['masked_image'] = rearrange(
            batch['masked_image'], 'b h w c -> b c h w').to(
                memory_format=torch.contiguous_format).float()
        return log
